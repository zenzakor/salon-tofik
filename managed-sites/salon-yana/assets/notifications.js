...
async function markRead(id, rerender = true) {
    if (!id) return;
    await updateDoc(doc(db, "site_salon_yana_Notifications", id), {
        read: true,
...
async function markAllRead() {
    ...
    unread.forEach((notification) => {
        batch.update(doc(db, "site_salon_yana_Notifications", notification.id), { read: true, readAt });
...
async function deleteNotification(id) {
    ...
    await deleteDoc(doc(db, "site_salon_yana_Notifications", id));
...
function startListening(targetId, targetType) {
    ...
    const notificationsQuery = query(collection(db, "site_salon_yana_Notifications"), where("userId", "==", targetId));
...
async function createNotification(payload) {
    ...
    return addDoc(collection(db, "site_salon_yana_Notifications"), {
